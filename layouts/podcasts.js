...
function Card({ podcast }) {
  return (
    <div
      className="w-full shadow-lg  bg-white rounded-lg sm:rounded-2xl p-1 sm:p-2.5 hover:shadow-md">
      {podcast.wip === true
        ?
        (<div className="flex flex-col items-center relative h-full cursor-not-allowed">
        ...
        : (
          ...
          <NextLink href={`/podcasts/${podcast.slug}`}>
            <div className="flex flex-col items-center relative h-full cursor-pointer">
              <img className="rounded-md" alt='podcast-tile' src={podcast.image} />
              <div
                className="flex text-gray-600 text-sm md:text-lg font-medium p-2 text-center h-full items-center">
                {podcast.title}
