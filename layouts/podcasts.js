    ...
    <div
      ...
      <NextLink href={`/podcasts/${podcast.slug}`}>
        <div className="flex flex-col relative h-full">
          ...
          <div
            className="text-gray-600 text-sm md:text-lg font-medium p-2 text-center h-full items-center flex">
            {podcast.title}
          </div>
          <div className="absolute italic opacity-0 hover:opacity-95 transition-opacity md:p-5 top-0 bottom-0 right-0 left-0 flex items-center text-center md:text-lg bg-gray-50 bg-opacity-95 rounded-md text-gray-800">{podcast.description}</div>
        </div>
      </NextLink>
    </div>
  )
